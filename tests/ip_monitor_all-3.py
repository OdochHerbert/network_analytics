...
def monitor_bandwidth(duration_seconds, output_file):
    try:
        ...
        with open(output_file, 'w', newline='') as csvfile:
            ...
            csvwriter.writerow(['IP Address', 'MAC Address', 'Bytes In'])
            ...
            for packet in capture.sniff_continuously():
                if 'ip' in packet:
                    ...
                    if ip_address in ip_info:
                        ip_info[ip_address]['Bytes In'] += bytes_in
                    else:
                        ip_info[ip_address] = {'MAC Address': mac_address, 'Bytes In': bytes_in}
            ...
            for ip, info in ip_info.items():
                csvwriter.writerow([ip, info['MAC Address'], info['Bytes In']])
